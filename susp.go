...
// SystemUseEntry represents a single System Use entry (SUSP-112 4.1)
...
// Data returns the data portion of the entry (after the 4-byte header)
func (e SystemUseEntry) Data() []byte {
	return e[4:]
}

// Type returns the 2-character type signature of the entry
func (e SystemUseEntry) Type() string {
	return string(e[:2])
}

// ExtensionRecord represents an ER (Extension Reference) record
...
// ExtensionRecordDecode decodes an ER entry (SUSP-112 5.5)
...
// SPRecord represents an SP (SUSP Indicator) record
type SPRecord struct {
	BytesSkipped uint8
}

// SPRecordDecode decodes an SP entry (SUSP-112 5.3)
...
// SystemUseEntrySlice is a slice of SystemUseEntry
type SystemUseEntrySlice []SystemUseEntry

// GetExtensionRecords returns all ER (Extension Reference) records
...
// ContinuationEntry represents a CE (Continuation Area) record (SUSP-112 5.1)
...
// SUSP entry type constants
const (
	SUETypeContinuationArea          = "CE"
	SUETypePaddingField              = "PD"
	SUETypeSharingProtocolIndicator  = "SP"
	SUETypeSharingProtocolTerminator = "ST"
	SUETypeExtensionsReference       = "ER"
	SUETypeExtensionSelector         = "ES"
)
...
func splitSystemUseEntries(data []byte, ra io.ReaderAt) ([]SystemUseEntry, error) {
	...
	for len(data) > 0 {
		...
		if entry.Type() == SUETypeContinuationArea {
...
// SUSPMetadata contains metadata about SUSP extensions for a directory
type SUSPMetadata struct {
	Offset       uint8
	HasRockRidge bool
}

// Clone returns a copy of the SUSPMetadata
