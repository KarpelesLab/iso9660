...
const (
	// RockRidgeIdentifier is the identifier for Rock Ridge extensions
	RockRidgeIdentifier = "RRIP_1991A"
	// RockRidgeVersion is the version of Rock Ridge extensions supported
	RockRidgeVersion = 1
)

// RockRidgeNameEntry represents an NM (Alternate Name) record
...
// GetRockRidgeName returns the Rock Ridge alternate name if available
func (s SystemUseEntrySlice) GetRockRidgeName() string {
	...
	for _, entry := range s {
		...
		if entry.Type() == "NM" {
			nm := unmarshalRockRidgeNameEntry(entry)
			name += nm.Name
		}
	}
...
// GetPosixAttr returns the POSIX file attributes from the PX record
func (s SystemUseEntrySlice) GetPosixAttr() (fs.FileMode, error) {
	for _, entry := range s {
		if entry.Type() == "PX" {
			...
			return unmarshalRockRidgeAttrEntry(entry)
		}
	}
...
func unmarshalRockRidgeAttrEntry(e SystemUseEntry) (fs.FileMode, error) {
	rrMode, err := UnmarshalUint32LSBMSB(e.Data()[0:8])
	if err != nil {
		return 0, fmt.Errorf("unmarshal RR PX entry: %w", err)
	}
...
func unmarshalRockRidgeNameEntry(e SystemUseEntry) *RockRidgeNameEntry {
