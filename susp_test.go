...
func TestSUSPMetadataClone(t *testing.T) {
	// Test nil clone
	var nilMeta *SUSPMetadata
	...
	assert.Equal(t, meta.Offset, cloned.Offset)
	assert.Equal(t, meta.HasRockRidge, cloned.HasRockRidge)

	// Ensure it's a copy, not the same pointer
	cloned.Offset = 10
	assert.NotEqual(t, meta.Offset, cloned.Offset)
}

func TestSystemUseEntryMethods(t *testing.T) {
	entry := SystemUseEntry{'P', 'X', 44, 1, 0, 0, 0, 0}

	assert.Equal(t, "PX", entry.Type())
	assert.Equal(t, 44, entry.Length())
	assert.Equal(t, []byte{0, 0, 0, 0}, entry.Data())
}
